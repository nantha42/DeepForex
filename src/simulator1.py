...
class Simulator:
    ...
    def draw(self):
        if time.time()-self.t1 >= .1:
            ...
            for i in range(window.size(-1)):
                ...
                if o > c:
                    #bear candle
                    bear_candle = py.Rect(graph_x + i*each_bar_width + bar_left_space, graph_y + (maximum-o)*1e5*window_scale ,bar_width,(o-c)*1e5*window_scale )
                    py.draw.rect(self.win,(255,0,0),bear_candle)
                if o <= c:
                    #bull candle
                    bull_candle = py.Rect(graph_x + i*each_bar_width + bar_left_space, graph_y + (maximum-c)*1e5*window_scale ,bar_width,( c-o )*1e5*window_scale )
    ...
    def manage(self):
        ...
        if self.state=='C':
            if self.buy_position != None:
                self.balance = self.balance +  (self.window[:,-1][0]-self.buy_position)*1e5*0.1 -self.transaction_cost 
            ...
            if self.sell_position != None:
                self.balance = self.balance +  (self.sell_position - self.window[:,-1][0])*1e5*0.1 -self.transaction_cost
    ...
    def step(self,action):
        ...
        if self.order_index != None:
            ...
            if self.data_index - self.order_index >= 40:
                reward = -1
        else:
...
if __name__ == '__main__':
    s = Simulator(False)
    s.set_data(models_dir+"../Dataset/EURUSD30min2015-17.csv")
    # print(s.step(1))
